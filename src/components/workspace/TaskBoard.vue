...
defineEmits<{ changeStatus: [task: WorkspaceTask, status: TaskStatus]; uploadEvidence: [task: WorkspaceTask, file: File]; openTask: [task: WorkspaceTask] }>()
...
<template>
  <div class="task-board">
    <article v-for="task in tasks" :key="task.id" class="task-row">
      <button class="task-main" :data-testid="`open-task-${task.id}`" @click="$emit('openTask', task)"><span>{{ task.taskType }}</span><strong>{{ task.title }}</strong><small>责任人 {{ task.ownerName }} · 计划 {{ task.plannedDate }}</small></button>
